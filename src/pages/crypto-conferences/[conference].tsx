...
import MonthlyConferencePage from './[month]';
import { Helmet } from 'react-helmet-async';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { CalendarIcon, MapPinIcon, ExternalLinkIcon, ArrowLeftIcon, UsersIcon, ClockIcon } from 'lucide-react';
...
// Find conference by year and slug
const findConferenceBySlug = (year: string, slug: string): Conference | null => {
  return conferences.find(conf => {
    const confYear = new Date(conf.date).getFullYear().toString();
    const confSlug = generateSlug(conf.name);
    return confYear === year && confSlug === slug;
  }) || null;
};
...
export function ConferencePage() {
  ...
  // Check if slug is a month name - if so, render the monthly page
  const monthNames = [
    'january', 'february', 'march', 'april', 'may', 'june',
    'july', 'august', 'september', 'october', 'november', 'december'
  ];

  if (monthNames.includes(slug.toLowerCase())) {
    return <MonthlyConferencePage />;
  }

  const conference = findConferenceBySlug(year, slug);
  ...
  const isUpcoming = confDate >= new Date('2025-09-13');

  // Get related conferences (same type or location)
  const relatedConferences = conferences
    .filter(conf =>
      conf.name !== conference.name &&
      (conf.type === conference.type ||
       conf.location.split(',').slice(-1)[0].trim() === conference.location.split(',').slice(-1)[0].trim())
    )
    .slice(0, 3);
  ...
  return (
    <>
      <Helmet>
        <title>{conference.name} {year} | Crypto Conferences | Perception</title>
        ...
        <div className="mx-auto max-w-7xl px-4 pt-24 pb-8 sm:px-6 lg:px-8">
            ...
            <div className="relative px-8 py-16 sm:px-12 sm:py-20 lg:px-16">
              <div className="max-w-4xl mx-auto text-center text-white">
                <Badge
                  ...
                  className={`mb-4 ${isUpcoming ? 'bg-green-600 hover:bg-green-700' : 'bg-gray-600'}`}
                >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <Card>
                ...
                <CardContent className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <h4 className="font-semibold text-sm text-muted-foreground mb-2">EVENT TYPE</h4>
                    ...
                    <div>
                      ...
                      <p className="text-sm">{conference.duration}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      <h4 className="font-semibold text-sm text-muted-foreground mb-2">DATE</h4>
                      <p className="text-sm">{conference.dateDisplay}</p>
                    </div>
                  </div>

                  <div className="pt-4 border-t">
                    <h4 className="font-semibold text-sm text-muted-foreground mb-3">ABOUT THIS CONFERENCE</h4>
                    <p className="text-sm text-muted-foreground leading-relaxed">
                      {conference.name} is a {conference.type.toLowerCase()} conference taking place in {conference.location}.
                      This event brings together industry leaders, developers, investors, and enthusiasts to discuss the latest trends
                      and developments in the cryptocurrency and blockchain space.
                    </p>
                  </div>
                </CardContent>
              </Card>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        This conference is ideal for professionals working in {conference.type.toLowerCase()},
                        blockchain developers, cryptocurrency investors, financial institutions, and anyone interested
                        in the future of digital assets.
                      </p>
                    </div>
                    <div>
                      <h4 className="font-semibold mb-2">Expected Topics</h4>
                      <p className="text-sm text-muted-foreground">
                        Sessions will likely cover regulatory developments, technological innovations, market trends,
                        investment strategies, and the practical applications of blockchain technology in various industries.
                      </p>
                    </div>
                  </div>
                </CardContent>
            ...
            <div className="space-y-6">
              ...
              <Card>
                <CardHeader>
                  <CardTitle>Conference actions</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  ...
                  <p className="text-xs text-muted-foreground text-center">
                    Registration and official website links are managed by the conference organizers.
                  </p>
                </CardContent>
              </Card>
              ...
              {relatedConferences.length > 0 && (
                <Card>
                  <CardHeader>
                    <CardTitle>Related conferences</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    {relatedConferences.map((related) => (
                      <div key={`${related.date}-${related.name}`} className="border-b border-border pb-4 last:border-b-0 last:pb-0">
                        <Link
                          to={generateConferenceUrl(related)}
              ...
              <Card>
                ...
                <CardContent>
                  <Link
                    to={`/crypto-conferences/${year}/${conference.monthYear.split(' ')[0].toLowerCase()}`}
                    className="block hover:bg-accent/50 p-3 rounded-lg transition-colors border border-border"
                  >
                    <h4 className="font-medium mb-2">View All {conference.monthYear} Events</h4>
                    <p className="text-xs text-muted-foreground">
                      See all cryptocurrency and blockchain conferences happening in {conference.monthYear}
                    </p>
                  </Link>
                </CardContent>
              </Card>
